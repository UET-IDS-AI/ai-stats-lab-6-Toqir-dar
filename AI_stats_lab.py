...
def bernoulli_mle_with_comparison(data, candidate_thetas=None):
    ...
    log_likelihoods = {theta: bernoulli_log_likelihood(data, theta) for theta in candidate_thetas}
...
def poisson_log_likelihood(data, lam):
    ...
    return float(sum(x * math.log(lam) - lam - math.lgamma(x + 1) for x in data))
